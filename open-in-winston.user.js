// ==UserScript==
// @name         Open in Winston
// @version      1.0.0
// @author       DivineOpinion
// @match        *://*.reddit.com/*
...
function openInWinston() {
	const winstonUrl = convertToWinstonUrl(window.location.href);
	if (winstonUrl) {
		window.location.href = winstonUrl;
	}
}

function convertToWinstonUrl(urlString) {
...
function processSearchResults() {
	const searchResults = document.querySelectorAll('a:not([data-winston-listener-added])');
	searchResults.forEach(link => {
		const href = link.href;
		if (!href) return;
		
		if (href.match(regexRedditIDs)) {
			const winstonUrl = convertToWinstonUrl(href);
			if (winstonUrl) {
				link.setAttribute('data-winston-listener-added', 'true');
